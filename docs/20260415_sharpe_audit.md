> ## 📌 이 문서의 현재 지위
>
> **작성 2026-04-15.** `20260415_analy.md` Tier1 §1(Sharpe 연환산 오류)의 상세 감사 기록입니다.
> **지적된 오류는 수정 완료**됐습니다 — `MetricsCalculator` 가 일별 equity curve 기반으로
> 재계산하도록 바뀌었고(기존 per-trade × √365 오류), `MetricsCalculator.java` 가 이 문서를
> 근거로 인용하고 있습니다.
>
> 따라서 **"현재 Sharpe 계산이 틀렸다"는 근거로 쓰면 안 됩니다.** 당시 무엇이 왜 틀렸고
> 어떤 기준으로 고쳤는지를 이해하기 위한 배경 문서입니다.
> 해소 내역은 [`docs/CHANGELOG.md`](CHANGELOG.md) 참조.
>
> 코드가 인용하는 문서이므로 옮기거나 이름을 바꾸지 마십시오.
