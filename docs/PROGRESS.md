...
### 🟡 [사용자 결정 필요] time stop 재활성화 — 근거는 강해졌으나 코드로 정할 일이 아님

- 전 세션 `max_hold_hours=0`(비활성). 세션 40 XRP가 **27시간째 −0.65%** 로, **SELL 게이트(본전 근처)와 SL/TP 사이 어디에도 걸리지 않는 사각지대**에 있다.
- 08-03 노트의 근거("①②로 유령 포지션 루프가 자동 복구되니 되돌릴 수 있다")는 **①②가 아직 미발화(표본 0)** 라 확인되지 않았다. 대신 위 XRP 사례가 **더 직접적인 근거**다 — time stop은 이 사각지대의 유일한 탈출구다.
- **[ ] 결정 필요** — 켠다면 `max_hold_hours` 값(예: 24h). **자본 회전 정책 변경이라 임의로 켜지 않았다.** 값만 정해주면 운영 DB UPDATE로 즉시 적용 가능(마이그레이션·배포 불필요).
