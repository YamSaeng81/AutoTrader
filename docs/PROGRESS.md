...
2. 🔴 **손절폭 A/B (`SL_ATR_MULTIPLIER` 1.5 → 2.5)** — §11. 실전 투입 여부를 가르는 단일 최대 레버.
   승률 4%p만 올리면 손익분기를 넘는다. 다른 어떤 튜닝보다 이걸 먼저 돌릴 것.
3. 🔜 **MTF_BTC_STRICT 비활성화 실행** — §15. `scripts/disable_duplicate_strategies.sh` (원인 규명·테스트 고정 완료).
4. **H1 축소 범위 결정** — §9. 승률 0% 3전략(ICHIMOKU_V2·MTF_CONFIRMED·MOMENTUM_ICHIMOKU)의 H1만 우선 중지 권장.
