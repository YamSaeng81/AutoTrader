...
> **마지막 갱신**: 2026-09-01 오후 — 🟢 **함대 점검 이상 없음** · 🔴 **WS 실시간 감시가 오늘에서야 실제 반영** · 🔴 **LLM 응답 본문 유실 버그 수정**(SIGNAL_ANALYSIS 27건 중 24건이 비어 있었다).
...
| 날짜 | 무엇을 볼 것인가 |
|---|---|
| **09-01 (월) 오전** | ① **게이트 OFF 후 거래가 돌아왔는지** — 하루 26~37건 수준이면 정상, 여전히 한 자리면 다른 원인이 있다. ② **WS 실시간 감시가 페이퍼에 걸리는지** — 08-31 배포의 핵심인데 아직 미검증(SL/TP 청산이 0건이라 증거가 없었다). 확인법: `[Dynamic] 실시간 손절(WS)` / `실시간 익절(WS)` 로그가 **DYN_PAPER 세션 id 로 찍히는지**(이전엔 REAL 만), 또는 페이퍼 SL/TP 청산의 `closed_at` 초 단위가 60초 격자에 안 걸리는지. ③ 불변식 3종·②, 최소 이력 가드, 노출 상한 2. |
