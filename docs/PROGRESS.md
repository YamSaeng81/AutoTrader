...
> **마지막 갱신**: 2026-08-26 — 운영 전면 점검. 진입 게이트 역선택 발견(노출 상한 PAPER 면제) + 캐시 전환 회귀 자체 발견·수정 + 스로틀 병목 수정(아래 섹션들).
...
- `:web-api:test` **66개 클래스 전부 그린**. **미배포**.
...
### 🔴 후속 — 이 전환이 만든 회귀를 같은 날 발견·수정

배포 직후 운영 점검에서, **캐시가 lookback 창을 못 덮는 코인은 전략이 500개가 아니라 캐시에 있는
만큼만 받는다**는 것을 발견했다. 전환 전에는 매 틱 REST로 500개를 새로 받았기 때문에 드러나지
않던 문제다.

| 코인 | TF | 캐시 행 수 | 창 시작점 대비 부족 |
|---|---|---|---|
| KRW-LIT | M15 | **180** | 80시간 |
| KRW-PROM | H1 | **334** | 166시간 |

EMA200 계열은 닫힌 캔들 **201개 미만이면 구조적으로 신호를 못 낸다**(`SYNC_CANDLE_COUNT` javadoc에
기록된 바로 그 실패 양식). LIT은 세션 61이 실제 포지션을 들고 있는 코인이었다.
