...
| 파일 | 변경 내용 |
|------|-----------|
| `OrderExecutionEngine.java` | `submitToExchange()` 시장가 매도 시 `volume.setScale(8, RoundingMode.DOWN)` 추가 |

> **근본 원인**: Upbit `price`타입 매수(KRW 총액 지정) 체결 시 `executed_volume`이 8자리 초과로 반환됨. 예: 8,000 KRW ÷ 3,437,000 = `0.0023276118...` (10자리 이상). 이 값을 그대로 ask(매도) volume으로 전송하면 Upbit이 `invalid_volume_ask` 400 오류 반환. `DOWN`(내림) 처리로 실제 보유량을 절대 초과하지 않음.
