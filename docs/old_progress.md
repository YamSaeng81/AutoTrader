> # 🛑 이 파일은 과거 이력입니다 — 현재 상태가 아닙니다
>
> **아래 본문은 `# CryptoAutoTrader — PROGRESS.md` 로 시작하고 "이 파일을 먼저 읽어
> 현재 상태를 파악한다"고 적혀 있지만, 그것은 이 파일이 현행 PROGRESS 였던 시절의
> 문장입니다.** 파일명만 `old_progress.md` 로 바뀐 채 본문이 그대로 남아 오독을 부릅니다.
>
> **현재 상태는 [`docs/PROGRESS.md`](PROGRESS.md) 를 보십시오.**
> 지금 무엇을 해야 하는지는 [`docs/NEXT.md`](NEXT.md).
>
> 이 파일(472KB)은 2026-08-06 이전 상세 이력의 백업입니다. **날짜로 검색해서 당시 근거와
> 재현 과정을 찾을 때만** 사용하고, 여기 적힌 수치·상태·"다음 할 일"을 현재로 읽지 마십시오.
>
> — 2026-09-15 문서 정리에서 추가
...
[docs/주요전략분석_v20260430.md](./archive/주요전략분석_v20260430.md) — `COMPOSITE_BREAKOUT`,
...
> 분석 문서([docs/주요전략분석_v20260430.md](./archive/주요전략분석_v20260430.md))의 한계를
...
# 📦 2026-08-18 ~ 2026-08-19 아카이브 (PROGRESS.md 에서 이관)

> 2026-08-19, `PROGRESS.md` 가 다시 1,800줄을 넘어 날짜별 상세 이력을 여기로 옮겼다.
> 이 구간의 핵심은 **데이터 신뢰성**이다 — 규칙 지문(V71) 도입, 매도 정산 멱등화(V72),
> 청산 사유 구조화(V73), 세션별 A/B 파라미터(V74). 상세 근거·재현 과정은 아래를 날짜로 검색할 것.
...
# 📦 2026-09-04 이관 — PROGRESS.md 에서 옮긴 상세 이력

> PROGRESS.md 가 2,655줄로 불어나 08-18~08-28 구간을 이관했다.
> 결론과 현재 유효한 규칙은 PROGRESS.md 에 남아 있다.
...
# 📦 2026-09-18 이관분 — PROGRESS.md 에서 옮겨 온 상세 이력

> 2026-09-18 정리. PROGRESS.md 가 2,582줄까지 불어나 "지금 무엇을 해야 하는가"가 묻혔다.
> 아래는 **결론이 이미 반영됐거나 기간이 지나 행동을 바꾸지 않는** 구간이다.
> 근거·재현 과정이 필요하면 **날짜로 검색**할 것.

> ⚠️ 이 구간의 백테스트·WF 숫자는 전부 **규칙 v1/v2** 산출물이다. 09-18 에 백테스트가 재는
> 대상 자체가 바뀌어(전략 구성 통일 · look-ahead 제거 · 손익 정의 변경) 무효화됐으니,
> 여기 적힌 성과·판정을 현재 판단 근거로 인용하지 말 것.
