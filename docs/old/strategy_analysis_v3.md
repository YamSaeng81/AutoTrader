...
> ⚠️ **참고**: 위 공식은 Fixed Fractional 방식이다.  
> Kelly Criterion(`f* = (bp - q) / b`)은 승률과 페이오프 비율 기반으로 최적 베팅 비율을 산출하는 별도 공식이며, 실전에서는 변동성이 크므로 **Half-Kelly**(Kelly 결과의 50%)를 적용하는 것이 보수적 접근이다.  
> 초기에는 Fixed Fractional로 시작하고, 충분한 거래 데이터(100+ 거래) 확보 후 Kelly 도입을 검토한다.
...
```
상관관계 높은 자산 동시 포지션 제한:
  - BTC + ETH 동시 롱 → 실질 포지션 1.5배로 간주
  - 상관계수 > 0.7인 자산 쌍 → 하나의 포지션 슬롯으로 취급
  - 예: Max Open Trades = 3이어도 BTC롱 + ETH롱 = 슬롯 2개 소모
```
...
[1] MARKET DETECTOR
    ...
    ├─ ATR Spike (+ ADX < 25)? → VOLATILITY
    ├─ ADX 20~25? → TRANSITIONAL (직전 Regime 유지)
    └─ Regime 전환 시 Hysteresis 적용 (3캔들 유지 확인)
    ↓
[2] STRATEGY SELECTOR
    ├─ TREND → Supertrend + EMA
    ├─ RANGE → Bollinger + RSI + Grid
    ├─ VOLATILITY → ATR Breakout
    └─ TRANSITIONAL → 직전 활성 전략 유지 + 포지션 축소
    ↓
[3] SIGNAL ENGINE (가중 투표)
    ├─ 선택된 전략들의 신호 생성
    └─ Weighted Voting (전략별 신뢰도 가중치)
    ↓
[4] RISK ENGINE
    ├─ Position Size 계산
    ├─ Correlation Risk 확인
...
| 🔴 높음 | **상태 미보존** | 어떤 그리드에서 이미 매수했는지 추적하지 않음. 동일 레벨에서 중복 매매 발생 가능 → **버그급 문제, 즉시 수정 필요** |
...
| 심각도 | 문제 | 설명 |
|--------|------|------|
| 🟡 중간 | **호가 스푸핑 미감지** | 대형 호가가 실행 직전 취소되는 스푸핑/레이어링을 구분할 수 없음. 호가 스냅샷만으로는 근본적 해결이 어려우며, **호가 변화율(delta) 추적**이 필요함 |
...
4. **호가 Delta 추적**: 연속 스냅샷 간 호가 추가/취소 패턴 분석으로 스푸핑 필터링
...
| 3 | **Grid 상태 미보존** | 그리드 레벨별 진입/청산 이력 추적 구현 (중복 매매 방지) |
| 4 | **위험 신호 검증** | 상충하는 신호(Bollinger SELL + Supertrend BUY) 테스트 |

**Phase 1 체크리스트:**
- [ ] Supertrend 코드 오류 수정
- [ ] Grid 초기화 안전화
- [ ] Grid 상태 추적 구현 (StatefulStrategy)
- [ ] 상충 신호 테스트 케이스 작성
...
class MarketRegimeDetector {
  ...
  Regime detect(List<Candle> candles) {
    ...
    double bbLow20 = calculateBandwidthPercentile(candles, 20); // 동적: 최근 N기간 하위 20%
    double atr = calculateATR(candles);
    double atrMA = calculateATRMovingAverage(candles, 20);       // 동적: ATR 20기간 이동평균
    ...
    if (adx > 25) {
      ...
      // ATR spike이면서 TREND가 아닌 경우만 VOLATILITY
    ...
    if (detected != previousRegime) {
      ...
      if (regimeHoldCount < HYSTERESIS_CANDLES) {
        return previousRegime; // 아직 전환하지 않음
      }
    }
...
> **핵심 설계 결정:**
> - `TRANSITIONAL` 상태 추가: ADX 20~25 구간에서 직전 Regime 유지
> - **Hysteresis**: Regime 전환 시 3캔들 연속 유지 확인 (잦은 전환 방지)
> - `bbLow20Period` → `calculateBandwidthPercentile()`: 하드코딩 대신 동적 퍼센타일
> - `atrHigh` → `atrMA * 1.5`: 하드코딩 대신 ATR 이동평균 대비 배수
> - VOLATILITY 판단에 `adx < 25` 조건 추가: TREND와 VOLATILITY 동시 트리거 방지
...
class RiskEngine {
  ...
  boolean canOpenPosition(List<OpenPosition> positions, int maxSlots) {
    return effectiveSlots(positions) < maxSlots;
  }
}
```
...
- [ ] TRANSITIONAL 상태 + Hysteresis 로직 구현
- [ ] ADX, Bollinger Bandwidth 지표 확인 및 퍼센타일 함수 구현
- [ ] Risk Engine 로직 검증 (Fixed Fractional)
- [ ] Correlation Risk 매트릭스 구현
- [ ] 단위 테스트: Regime 전환 시나리오 (TREND→RANGE→VOLATILITY)

### Phase 3: Strategy Selector & Composite Strategy (가중 투표)

**기간**: 2주 | **영향도**: 🟠 높음

```java
class StrategySelector {
  List<WeightedStrategy> selectStrategies(MarketRegime regime) {
    switch(regime) {
      case TREND:
        return List.of(
          new WeightedStrategy(new SupertrendStrategy(...), 0.5),  // 메인
          new WeightedStrategy(new EmaStrategy(...), 0.3),         // 보조
          new WeightedStrategy(new AdxIndicator(...), 0.2)         // 확인
        );
      case RANGE:
        return List.of(
          new WeightedStrategy(new BollingerStrategy(...), 0.4),
          new WeightedStrategy(new RsiStrategy(...), 0.4),
          new WeightedStrategy(new GridStrategy(...), 0.2)
        );
      case VOLATILITY:
        return List.of(
          new WeightedStrategy(new AtrBreakoutStrategy(...), 0.6),
          new WeightedStrategy(new StochasticRsiStrategy(...), 0.4)
        );
      case TRANSITIONAL:
        // 직전 활성 전략 유지, 포지션 크기 50% 축소
...
class CompositeStrategy implements Strategy {
  StrategySignal evaluate(List<Candle> candles, Map<String, Object> params) {
    List<WeightedStrategy> selected = selector.selectStrategies(regime);
    
    // 가중 투표 (Weighted Voting)
    double buyScore = 0, sellScore = 0;
    for (WeightedStrategy ws : selected) {
      StrategySignal signal = ws.strategy.evaluate(candles, params);
      double weight = ws.weight * signal.getConfidence(); // 가중치 × 신뢰도
      
      if (signal.isBuy()) buyScore += weight;
      else if (signal.isSell()) sellScore += weight;
    }
    
    // 의사결정
    double threshold = 0.6; // 가중 합산이 60% 이상이어야 신호 유효
    if (buyScore > threshold && sellScore < 0.2) return STRONG_BUY;
    if (buyScore > 0.4 && sellScore < 0.2) return WEAK_BUY;
    if (sellScore > threshold && buyScore < 0.2) return STRONG_SELL;
    if (sellScore > 0.4 && buyScore < 0.2) return WEAK_SELL;
    
    // 신호 상충 (BUY vs SELL 모두 높음) → 무조건 HOLD
    return HOLD;
  }
}
```
...
> **핵심 설계 결정:**
> - 단순 다수결 → **가중 투표(Weighted Voting)** 전환
> - 전략별 가중치: Supertrend(0.5) > EMA(0.3) > ADX(0.2) 등 신뢰도 반영
> - **Confidence 레벨과 연동**: 가중치 × 신호 신뢰도 = 최종 투표 점수
> - **상충 신호 자동 HOLD**: BUY/SELL 점수가 모두 높으면 진입하지 않음

**Phase 3 체크리스트:**
- [ ] WeightedStrategy 래퍼 클래스 구현
- [ ] Strategy Voting 알고리즘 구현 (가중 투표)
- [ ] CompositeStrategy 인터페이스 설계
- [ ] 신호 조합 규칙 명세화 (threshold 조정 테스트)
- [ ] TRANSITIONAL Regime에서의 포지션 축소 로직 검증
...
| 7 | **Orderbook** | Trade Flow + 호가 Delta 추적 결합 | ⭐⭐⭐ |

**Phase 4 체크리스트:**
- [ ] Supertrend ATR 사전 계산 리팩토링
- [ ] EMA Cross ADX 필터 통합
- [ ] Bollinger Squeeze 감지 로직
- [ ] RSI 피봇 기반 다이버전스 구현
- [ ] ATR Breakout 거래량 필터 추가
- [ ] Orderbook 호가 Delta 추적 구현

### Phase 5: 신호 확장 & 타임프레임 + 백테스트
...
| 2 | **Confidence 레벨** | 신호 신뢰도 (0.0 ~ 1.0) → Phase 3 Weighted Voting과 연동 |
| 3 | **Multi-Timeframe** | 상위 TF 트렌드 + 하위 TF 진입 |
| 4 | **타임프레임별 파라미터** | 1m/5m/1h/1d 별 자동 조절 |
| 5 | **백테스트 프레임워크** | 2023-2025년 BTC/ETH 데이터 기반 전체 시스템 검증 |

**Phase 5 체크리스트:**
- [ ] StrategySignal에 stopLoss/takeProfit/confidence 필드 추가
- [ ] Multi-Timeframe 데이터 파이프라인 구축
- [ ] 타임프레임별 파라미터 프리셋 정의
- [ ] 백테스트 엔진 구현 (Phase 1-4 전체 통합 검증)
...
```
Tier 1 (필수):
1. Supertrend      (추세 방향 = 큰 틀)         가중치: 0.5
2. RSI             (진입 타이밍 = 세밀한 조정)   가중치: 0.3
3. ATR Breakout    (손절 거리 = 리스크 관리)     가중치: 0.2

Tier 2 (보조):
4. Bollinger       (범위 확인)                  가중치: 0.4
5. VWAP            (가치 평가)                  가중치: 0.3

Tier 3 (선택):
6. Orderbook       (수급 확인, 실시간 필수)      가중치: 0.2
7. StochasticRSI   (빠른 타이밍, 스캘핑)         가중치: 0.2
...
[1] Market Regime 판단 (Hysteresis 적용)
  ...
  ↓
[2] 선택된 전략들의 신호 생성
  ├─ Supertrend: BUY/SELL/HOLD (confidence: 0.0~1.0)
  ├─ RSI: BUY/SELL/HOLD (confidence: 0.0~1.0)
  └─ ATR Breakout: BUY/SELL/HOLD (confidence: 0.0~1.0)
  ↓
[3] Weighted Voting (가중 투표)
  ├─ 가중 합산 > 0.6 → 강한 신호
  ├─ 가중 합산 > 0.4 → 약한 신호
  ├─ BUY/SELL 상충 (양쪽 모두 높음) → HOLD (진입 금지)
  └─ 모두 HOLD → HOLD
  ↓
[4] Risk Engine 확인
  ├─ Position Size 계산 (Fixed Fractional)
  ├─ Correlation Risk 체크 (상관자산 슬롯 합산)
  ├─ Daily Loss 체크
  ├─ Max Open Trades 체크 (effective slots 기준)
...
#### VOLATILITY 시장 (ATR spike, ADX < 25)
...
#### TRANSITIONAL 시장 (ADX 20~25)

```
- 직전 활성 전략 그룹 유지
- 모든 voting weight × 0.5 (신뢰도 감소)
- Position Size × 0.5 (리스크 축소)
- 신규 포지션 진입 제한 (기존 포지션 관리만)
```
...
| 조합 | 유형 | 적합 시장 | 신호 빈도 | 승률 기대 | 복잡도 | 특성 요약 |
|------|------|----------|-----------|-----------|--------|----------|
| **Supertrend + RSI + ATR** | 트렌드 팔로잉 | 트렌드장 | ⭐⭐ | ⭐⭐⭐⭐ | ⭐⭐ | 추세 방향에 순행, 횡보 시 신호 감소로 자연 필터링 |
| **EMA Cross + MACD + VWAP** | 스윙 트레이딩 | 추세 전환기 | ⭐⭐ | ⭐⭐⭐ | ⭐⭐ | 추세 초입 포착에 강하지만 횡보 시 whipsaw 위험 |
| **Bollinger + StochRSI + Orderbook** | 스캘핑 | 횡보/변동 | ⭐⭐⭐⭐ | ⭐⭐⭐ | ⭐⭐⭐ | 빈도 높고 소폭 수익, 수수료 관리가 핵심 |
| **Grid + Bollinger + RSI** | 그리드 트레이딩 | 횡보장 | ⭐⭐⭐⭐⭐ | ⭐⭐⭐ | ⭐⭐ | 범위 내 안정적이나 돌파 시 큰 손실 위험 |
| **RSI Div + MACD + Supertrend** | 반전 매매 | 바닥/천장권 | ⭐ | ⭐⭐⭐⭐⭐ | ⭐⭐⭐⭐ | 고승률이지만 신호 희소, 타이밍이 핵심 |
...
- 신호 상충 제거 → **거짓 신호 대폭 감소**
- Daily Loss Limit + Correlation Risk → **최악의 날도 통제 가능**

#### 수익성 향상
- Weighted Voting으로 **신호 신뢰도 향상**
...
> ⚠️ **수치 목표는 백테스트 완료 후 설정한다.**  
> 백테스트 없이 승률이나 연수익 목표를 사전에 제시하면 기대치 왜곡과 과적합(overfitting) 위험이 있다.

**Phase별 검증 기준:**

| Phase | 검증 항목 | 기준 |
|-------|----------|------|
| Phase 2 완료 | Regime 분류 정확도 | 수동 라벨 대비 70%+ 일치 |
| Phase 3 완료 | 신호 상충 비율 | 기존 대비 80%↓ |
| Phase 4 완료 | 개별 전략 승률 | 백테스트로 각 전략 단독 성과 측정 |
| Phase 5 완료 | 통합 시스템 성과 | 2023-2025 BTC/ETH 백테스트 → 수치 목표 도출 |
...
**핵심 개선 4가지**만 구현하면:
1. **Market Regime Detector** → 신호 충돌 제거 (TRANSITIONAL + Hysteresis로 안정적 전환)
2. **Risk Engine** → 손실 통제 (Fixed Fractional + Correlation Risk)
3. **Strategy Selector** → 맥락 기반 전략 선택 (가중 투표)
4. **Multi-Timeframe** → 진입점 정확도 향상

**단계적 구현**을 통해 **신뢰할 수 있는 자동매매 시스템**으로 발전할 수 있다.

**추천**: Phase 1-2 (버그 수정 + 시장 판단 + 리스크 관리)부터 시작하여, 3개월 내 완전 자동화 시스템 구축 목표. 단, 수치적 성과 목표는 Phase 5 백테스트 완료 후 데이터 기반으로 설정한다.