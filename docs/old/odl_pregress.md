...
| Phase 4 | **실전매매** (LiveTrading) | **~95%** — 백엔드/프론트엔드/보안/운영 기동 완료, 실거래 검증 미완 |
...
### 2026-03-17 작업 (Spring Security API 인증 추가)

#### 변경 내용

| 파일 | 변경 내용 |
|------|-----------|
| `web-api/build.gradle` | **`spring-boot-starter-security` 의존성 추가** |
| `web-api/.../config/ApiTokenAuthFilter.java` | **신규 생성** — `Authorization: Bearer <token>` 헤더 검증 (`OncePerRequestFilter`). 토큰 불일치 시 SecurityContext 미설정 → SecurityConfig에서 401 반환. 기본 토큰 사용 시 기동 시 경고 로그 출력 |
| `web-api/.../config/SecurityConfig.java` | **신규 생성** — CSRF 비활성, Stateless 세션, CORS 통합 관리(`CorsConfigurationSource` Bean). 공개 엔드포인트: `/api/v1/health`, `/api/v1/trading/health/exchange`, `/swagger-ui/**`, `/v3/api-docs/**`. 나머지 전체 인증 필요. 401/403 JSON 응답 커스텀 |
| `web-api/.../config/WebConfig.java` | **CORS 설정 제거** — Spring Security 추가 시 WebMvcConfigurer CORS보다 Security CORS가 먼저 적용되므로 `SecurityConfig.corsConfigurationSource()`로 일원화. 충돌 방지 |
| `web-api/src/main/resources/application.yml` | **`api.auth.token: ${API_AUTH_TOKEN:dev-token-change-me-in-production}` 추가** |
| `crypto-trader-frontend/src/lib/api.ts` | **Bearer 토큰 인터셉터 추가** — `NEXT_PUBLIC_API_TOKEN` 환경변수를 모든 Axios 요청에 자동 주입 |
| `crypto-trader-frontend/Dockerfile` | **`ARG NEXT_PUBLIC_API_TOKEN` + `ENV` 추가** — `npm run build` 전에 선언하여 Next.js 빌드 시점에 번들에 포함 (`NEXT_PUBLIC_*`는 런타임 주입 불가) |
| `docker-compose.prod.yml` | **`NEXT_PUBLIC_API_TOKEN`을 `environment` → `build.args`로 이동**. `API_AUTH_TOKEN` 백엔드 환경변수 추가. 운영 서버 정상 기동 확인 ✅ |

#### 핵심 구현 포인트

- **토큰 검증 방식**: Static Bearer Token — JWT 없이 단일 토큰 비교. 개인 프로젝트 단일 사용자 환경에 적합
- **NEXT_PUBLIC 빌드 타임 이슈**: Next.js `NEXT_PUBLIC_*` 변수는 `npm run build` 시 번들에 하드코딩됨. Docker `environment`가 아닌 `build.args`로 전달해야 프론트엔드에서 사용 가능
- **공개 엔드포인트**: 헬스체크 + Swagger는 토큰 없이 접근 가능
...
| 파일 | 변경 내용 |
|------|-----------|
| `crypto-trader-frontend/src/components/layout/MainContent.tsx` | **차트 가로 스크롤 수정** — `flex-1`에 `min-w-0 overflow-hidden` 추가. 기존에는 flex 아이템의 `min-width: auto` 기본값으로 인해 차트가 넓어지면 페이지 전체가 가로로 늘어나던 문제 해결 |
| `web-api/.../service/LiveTradingService.java` | **차트 데이터 조회 메서드 추가** — `getChartCandles()`: 세션 기간의 `CandleDataEntity` 반환, `getAllSessionOrders()`: 세션 전체 주문 반환 |
| `web-api/.../controller/TradingController.java` | **`GET /sessions/{id}/chart` 엔드포인트 추가** — PaperTradingController와 동일 구조; `PositionRepository` 주입 + `toOrderMap()` (SELL 주문 시 buyPrice/realizedPnl 포함) |
| `web-api/.../repository/paper/PaperPositionRepository.java` | **`findBySessionId()` 추가** — 상태 필터 없이 세션 전체 포지션 조회 |
| `web-api/.../service/PaperTradingService.java` | **`getAllPositions()` 추가** — OPEN+CLOSED 전체 포지션 반환 |
| `web-api/.../controller/PaperTradingController.java` | **positions에 `?status=` 파라미터 추가** (기본값 `OPEN`, `ALL` 지정 시 전체 반환); `toPositionMap()` `Map.of()` → `HashMap` 전환 + `realizedPnl`, `status`, `closedAt`, `entryPrice` 필드 추가 |
| `crypto-trader-frontend/src/lib/api.ts` | **`tradingApi.getSessionChart()` 추가**; `paperTradingApi.positions()` status 파라미터 추가 |
| `crypto-trader-frontend/src/app/trading/[sessionId]/page.tsx` | **가격 차트 섹션 추가** — 매수/매도 마커 + signalReason 툴팁 + 가로 스크롤 지원; **종료된 거래 이력 테이블 추가** — 진입가·수량·실현손익·수익률·진입/청산시각 |
| `crypto-trader-frontend/src/app/paper-trading/[sessionId]/page.tsx` | **`positions` API를 `status=ALL`로 변경** — OPEN/CLOSED 분리; **종료된 거래 이력 테이블 추가** |
...
- [x] `🔴 CRITICAL` **Phase 4 실전매매 배포** — `UPBIT_ACCESS_KEY`, `UPBIT_SECRET_KEY` 환경변수 설정, 운영 서버 기동 확인 (2026-03-17)
- [x] `🔴 CRITICAL` Spring Security / API 인증 추가 — Bearer Token 인증 구현 완료, 운영 서버 정상 기동 확인 (2026-03-17)
...
## 테스트 서버 명령어 (로컬 Windows)

```bash
# 1. DB + Redis 시작 (docker-compose.yml — db/redis만 포함)
docker compose up -d
...
# 3. 프론트엔드 로컬 실행 (포트 3000)
cd crypto-trader-frontend
npm run dev

# 중단
docker compose stop          # DB + Redis 중단
# 백엔드/프론트엔드는 Ctrl+C 로 중단
```
