...
### CI 가 6개월간 red 였다 — 원인 3건 모두 해소 (2026-09-15)

`e2e` job 을 추가하면서 CI 를 처음 돌려 보고 드러났다. **세 가지가 겹쳐 있었고 서로를
가리고 있었다.** 로컬 개발과 `git pull` + docker 배포만 쓰다 보니 아무도 확인하지 않았다.

| # | 원인 | 언제부터 | 왜 안 보였나 |
|---|---|---|---|
| 1 | `gradlew` 가 `100644`(실행 불가) | 저장소 초기 | Windows 는 `core.filemode=false` 라 실행 비트가 보존되지 않는다. 로컬에서는 무관하게 동작 |
| 2 | CI 가 테스트에 PostgreSQL 접속정보 주입 | 2026-03-23 | 테스트는 H2 인메모리를 쓴다. 환경변수가 yml 보다 우선해 URL 만 덮이고 드라이버는 H2 로 남아 `Driver org.h2.Driver claims to not accept jdbcUrl` 로 컨텍스트 전멸. 로컬 `./gradlew test` 는 env 가 없어 통과 |
| 3 | 프런트엔드 lint error 88건 | 누적 | `npm run build` 는 성공하므로 기능 문제로 보이지 않았다 |

**1번이 2번을 가리고 있었다.** `./gradlew` 가 실행되지 않아 테스트 단계에 도달한 적이
없었기 때문이다. 서비스 컨테이너를 제거해 기동 대기 30초가 사라지자 실패 시각이
41초 → 5초로 앞당겨졌고, 그 변화가 1번을 특정하는 단서가 됐다.

3번은 규칙을 끄지 않고 전부 타입으로 해소했다(`no-explicit-any` 86 +
`set-state-in-effect` 2). 상세는 커밋 `399fc5d`. 짚어둘 점은 **`any` 가 실제 결함을
가리고 있었다**는 것이다 — 타입을 붙이자 `PaperOrder` 의 필드 4개 누락, null 가능
값을 객체 인덱스로 쓰던 자리, LLM 집계 행과 목록 행의 키 이름 불일치가 컴파일러에
잡혔다. `chartInner() as any` 3곳은 애초에 캐스팅이 불필요했다.

**재발 방지**: `.github/workflows/ci.yml` 의 두 자리(env·services)에 경위를 주석으로
남겼다. 앞으로 CI 가 red 이면 방치하지 말 것 — 이번처럼 원인이 겹쳐 쌓인다.
